...
export function FontSizeToolbarButton() {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 rounded-md bg-muted/60 p-0.5">
      <button 
        onClick={() => handleFontSizeChange(1)}
        className="flex h-6 w-9 items-center justify-center rounded hover:bg-accent p-0"
        type="button"
      >
      ...
      <Popover open={isFocused} modal={false}>
        <PopoverTrigger asChild>
          <input
            className="h-6 w-9 bg-transparent px-0.5 text-center text-xs hover:bg-muted rounded cursor-pointer outline-none"
            value={displayValue}
            onBlur={() => {
              setIsFocused(false);
              handleInputChange();
